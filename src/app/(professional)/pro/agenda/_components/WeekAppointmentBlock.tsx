...
export function WeekAppointmentBlock({
  ...
  return (
    <button
      type="button"
      className={cn(
        "absolute left-0.5 right-0.5 overflow-hidden px-1 text-left transition-opacity hover:opacity-80 border-l-2 flex items-center gap-0.5", RADIUS.sm,
    ...
    >
      {isWalkIn && (
        <span className="shrink-0 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-amber-400/20 text-[7px] font-bold text-amber-600 dark:text-amber-400">W</span>
      )}
      {canMark && (
        <span
          className={cn(
      ...
      <p className="truncate text-[10px] font-medium leading-none">
        <span className="tabular-nums">{apt.appointment_time.slice(0, 5)}</span>
